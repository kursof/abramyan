...
int main()
{
	...
	*/
	...
	// клетка расположена в средней строке допустимого поля
	// клетка расположена в верхней строке допустимого поля
	// клетка расположена в нижней строке допустимого поля
	// (?)Очень спорный вопрос как ставить тут комментарии.
	// Но если заведомо известно, что выходить за 80/120 столбец нелья,
	// Тогда лучше дописать комментарий сверху. И в нем описать, что происходит.
	bool d = ((x2 == x1 && (y2 == y1 - 1 || y2 == y1 + 1)) || 
		(x2 == x1 + 1 && (y2 == y1 - 1 || y2 == y1 + 1 || y2 == y1)) ||
		(x2 == x1 - 1 && (y2 == y1 - 1 || y2 == y1 + 1 || y2 == y1)));
